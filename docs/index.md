...
!!! warning "This book is under active development"
    Chapters are being rewritten one at a time into a fuller format — intuition, technical background, and hand-worked numeric examples ahead of every block of code. Finished so far: **Chapter 1 (Variables and Types)** and **Chapter 2 (Struct Design Patterns)**. Everything else is still in its original, denser draft form and will be revised in the same style, chapter by chapter.
